...
log_file_path = r'/distribute/distributed-systems-gmm/logs/GroupManagement-3.log'
class GroupManagementTests(unittest.TestCase):
   ...
   def test_node(self):
       ...
       time.sleep(5)
       cmd= "{0} PATH -sid {3} &".format
       p=subprocess.Popen(['/usr/bin/python',PATH, '-sid' , '3'],stdout=subprocess.PIPE)
       ...
       with open(log_file_path, 'r') as file:
           for line in file:
               ...
               value = ' '.join(match)
                #print value
               assert "Started listening on port 50003",value
   ...
   def testfollower(self):
        ...
        with open(log_file_path, 'r') as file:
            for line in file:
                ...
                value = ' '.join(match)
                #print value
                assert "Starting Group Management Node Server with ID 3",value
